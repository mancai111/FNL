...
# ./run_curate_picks.sh 
# --o External/job003/ 
# --in_mics simulation_1/micrographs.star 
# --in_coords simulation_1/manualpick_sim1.star 
# --param1 150 
# --param2 1 
# --j 1 
...
if __name__ == '__main__':
    ...
    try:
        ...
        OUTPUT_DIR = 'External/job003/'
        MICROGRAPHS = 'simulation_1/micrographs.star'
        COORDS = 'simulation_1/manualpick_sim1.star'
        PARAM1 = 150 #box size
        PARAM2 = 1 #number of new micrographs to look at per run



        def sq(x0, y0, d,fom=-1000):
            # draws a square around particle at x0,y0 with edge length d. fom is for showing figure of merit on the image
        ...
        mrcpaths = []
        appaths = []
        with open(COORDS) as aps:
            # read autopick.star to get micrograph and autopick paths
            lines = aps.readlines()
            lines = lines[8::]
            for linei in lines:
                spliti = linei.split()
        ...
        ct = 1  # for testing
        mrcpaths_used = []
        for mrcpath, appath in zip(mrcpaths, appaths):
            # loop over all micrographs and coordinate files listed in autopick.star file
            skip = False
            #ct += 1
