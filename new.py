...
def get_coord(event):
    ...
    if x and y:
        ...
        if coords and len(coords) % 2 == 0:
            ...
            mid_point_output = (mid_point[0] * (1/scale), mid_point[1] * (1/scale))
            ...
            with open(f'{OUTPUT_DIR}/run.out','a') as f: 
                ...
                f.write(f"The center's position is {mid_point_output}\n")
                f.write('\n')
                coords.clear()
    else:
        with open(f'{OUTPUT_DIR}/run.out','a') as f: 
...
if __name__ == '__main__':
    ...
    try:
        ...
        PARAM2 = sys.argv[4] # scale
        PARAM3 = sys.argv[5] # how many micrographs
...
### Output a micrograph (clarity needs to be improve)
        ...
        used_micrographs = f'{OUTPUT_DIR}/circled_micrographs.out'

        mrcpaths_used = []

        if os.path.isfile(used_micrographs):
             pass
        else:
             with open(used_micrographs, 'w') as fp:
                pass
             
        with open(used_micrographs ) as used:
            l = used.readlines()
            for i in l:
                if 'mrc' in i:
                    mrcpaths_used.append(i.strip('\n'))

        mrcpaths = []
        with open(MICROGRAPHS) as aps:
            lines = aps.readlines()
            for i in lines:
                if i.strip():
                    mrc = i.split()[0]
                    if '.mrc' in mrc and mrc not in mrcpaths_used:
                        mrcpaths.append(mrc)


        if len(mrcpaths) == 0:
            print('No more micrographs to process.')
            with open(f'{OUTPUT_DIR}/RELION_JOB_EXIT_SUCCESS','w') as f:
                pass
        else:
            ct = 1

            for example_mrc in mrcpaths:
                ...
                with open(f'{OUTPUT_DIR}/run.out','a') as f: 
                    f.write(f'The micrograph has been downscaled by {scale}, calculations below are based on original\n')
                    f.write('\n')
...
#########################
        if len(mrcpaths) < int(PARAM3):
            print('The rest is done')
        else:
